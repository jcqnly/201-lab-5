...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  var sumWithTwoParam = sum(a, b)[0];
  var sumWithThreeParam = sum(sumWithTwoParam, c)[0];
  var multWithTwoParam = multiply(a, b)[0];
  var multWithThreeParam = multiply(multWithTwoParam, c)[0];
  var thirdMessage = (a + ' and ' + b + ' and ' + c + ' sum to ' + sumWithThreeParam + '.');
  var fourthMessage = ('The product of ' + a + ' and ' + b + ' and ' + c + ' is ' + multWithThreeParam + '.');
  return[sumWithThreeParam, multWithThreeParam, thirdMessage, fourthMessage];
}

// DONE: Here is the test for sumAndMultiply(); uncomment it to run it
testSumAndMultiply(4,7,5);
document.getElementById("sumAndMultiply").innerHTML = sumAndMultiply(4,7,5);
// Once you get the test passing, do an a-c-p cycle and synchronize the code between GitHub and your laptop. Don't forget to create a new branch for your work on the next question!
...
function sumArray(testArray) { //eslint-disable-line
  ...
  for(var i = 0; i < testArray.length; i++) {
    testArraySum = sum(testArray[i], testArraySum)[0];
  }
  var message = testArray + ' was passed in as an array of numbers, and ' + testArraySum + ' is their sum.';
  return [testArraySum, message];
}

// DONE: Here is the test for sumArray(); uncomment it to run it
testSumArray(testArray);
document.getElementById("sumArray").innerHTML = sumArray(testArray);
...
function multiplyArray(testArray) { //eslint-disable-line
  var totArrayProduct = 1;
  var arrayProduct = 0;
  for(var i = 0; i < testArray.length; i++) {
    arrayProduct = multiply(testArray[i], 1)[0];
    totArrayProduct = multiply(arrayProduct, totArrayProduct)[0];
  }
  var message = 'The numbers ' + testArray + ' have a product of ' + totArrayProduct + '.';
  return [arrayProduct, message];
}

//DONE: Here is the test for multiplyArray(); uncomment it to run it
testMultiplyArray(2,3,4);
document.getElementById("multipleArray").innerHTML = multiplyArray(testArray);
